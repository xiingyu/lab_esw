...
class RedBallDetector:
    def __init__(self, camera_index=1, focal_length=403, actual_diameter_meters=0.04267, neck_angle=60):
        self.cap = cv2.VideoCapture(camera_index)
        self.focal_length = focal_length
        self.actual_diameter_meters = actual_diameter_meters
        self.actual_area_meters = np.pi * (actual_diameter_meters / 2) ** 2
        self.neck_angle = neck_angle
        self.lower_red = np.array([170, 100, 150])
    ...
    def detect_and_calculate_distance(self, frame):
        ...
        if contours:
            ...
            case_number = self.determine_case(cX, cY, segment_width, segment_height)
            ...
            neck_angle_radians = np.radians(self.neck_angle)
    ...
    def determine_case(self, cX, cY, segment_width, segment_height):
