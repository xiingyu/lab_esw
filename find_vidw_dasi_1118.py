...
class MainAlgo() :
    ...
    def check_position(self):
		# 공의 x좌표 기준으로 판단(왼쪽 40%에 있는지)
        if self.red_circle[0][0] < int(self.img_size_x * 0.4) : 
            return "Left"
            # 공의 x좌표 기준으로 판단(오른쪽 40%에 있는지)
        elif self.red_circle[0][0] > int(self.img_size_x * 0.6) :
            return "Right"
        else :
            return "Center"
        
    def check_ypos(self):
        # 공의 y좌표 기준으로 너무 아래쪽(하단 30%)에 있는 지 확인
        if self.red_circle[0][1] > int(self.img_size_y * 0.7) :
            self.head_v_align -= 10
            self.serial_comm.send_data(self.head_v_align)
            self.delay(0.8)
            return "Bottom"
        else :
            return
        ...
        if self.main_state == "find_ball" :
            ...
            if self.high_view_flag == False :
                ...
                if self.red_ball_detection == True :
                    self.check_position()
                    self.check_ypos()
                    print(f"Detected ball position: {self.head_h_align}, ypos: {self.head_v_align}")
                    self.process_frame()
                    return  # 공 탐지 후 실행 종료
