        ...
        if self.red_ball_detection:
            ...
            if self.initial_commands_complete and not self.position_reached:
                ...
                if position == "Left":
                    ...
                    while not self.position_reached and current_command >= 1150:
                        # 명령어 전송
                        ...
                        if self.red_ball_detection:
                            ...
                            if position == "Center":
                                ...
                                # Center가 아니라면 명령어 값을 감소하여 조정 계속
                                current_command -= 10

                # Right 상태에서 현재 각도 조정
                elif position == "Right":
                    self.position_started = "Right"
                    current_command = 11110
                    while not self.position_reached and current_command <= 11150:
                        # 명령어 전송
                        self.serial_comm.send_data(current_command)
                        print(f"Sent command: {current_command} to adjust rightward")
                        ...
                        if self.red_ball_detection:
                            ...
                            if position == "Center":
                                ...
                                print("right2center")
                                break
                        # Center가 아니라면 명령어 값을 증가하여 조정 계속
                        current_command += 10
