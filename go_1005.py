...
class ObjectDetector:
    ...
    def check_alignment(self, red_circle, yellow_circle):
        if red_circle and yellow_circle:
            ...
            print("노란 공이 없으나, 빨간 공 기준으로 정렬 처리(RED_DIST로 이동).")
            self.state = "RED_DIST"
            return True, "Align (red only)!"
        return False, ""    

    def calculate_distance(self, radius_red):
        print("Executing calculate_dist")
        # 공의 실제 직경(미터)
...
detector = ObjectDetector(
    ...
    lower_yellow=np.array([10, 170, 170]),
