## update :: 2024.11.05 공이 안보이다가 보일 때, 어떻게 해야할 지 추가함
...
class MainAlgo:
    def __init__(self):
        ...
        self.frame_rate = 10
        self.current_command = 11100  # 초기 명령(좌우 목각도)
        self.current_neck = 1670 # 초기 명령(상하 목각도)
    ...
    def process_frame(self):
        ...
        if not self.red_ball_detection:
            self.current_neck = 1660 # 처음에 공 안보였으면 목각도 10도 더 내림 (시야는 더 좁아질듯)
            self.serial_comm.send_data(self.current_command)
            self.delay(0.5)

            # 프레임 업데이트 및 위치 재확인
            self.update_frame()  # 새로운 프레임 가져오기
            if self.red_ball_detection:  # 빨간 공이 감지되었는지 확인
                position = self.check_position(self.red_circle, self.img_size_x)
                print(f"Current position after adjustment: {position}")
            else:
                print("No ball detected after neck adjustment")
            return

        position = self.check_position(self.red_circle, self.img_size_x)

        if self.initial_commands_complete and not self.position_reached: # 자세 초기화 후 center가 아님
            if position == "Left":
                if self.position_started != "Left": # 초기 시작 위치가 업데이트 되지 않게 함!!
                    self.position_started = "Left"
                    self.current_command = 1197  # 초기 명령 값 설정
                ...
                if position == "Left":
                    ...
                    print("Error: Unexpected transition from left to right")
                    self.current_command += 1
                    return

            elif position == "Right":
                if self.position_started != "Right": # 초기 시작 위치가 업데이트 되지 않게 함!!
                    self.position_started = "Right"
                    self.current_command = 11103  # 초기 명령 값 설정
                ...
                if position == "Right":
                    ...
                    print("Set :: Right 2 center")
                    self.position_reached = True
                    return  # 목표에 도달하면 종료
                
                else:
                    print("Error: Unexpected transition from right to left")
                    self.current_command -= 1
                    return
    ...
    def finite_statemachine(self):
        ...
        if self.main_state == "find_ball":
            if self.first_frame:
                ...
                self.delay(0.1)
                self.first_frame = False # 초기화 동작 수행 후에 다시 안하려고 플래그
                self.initial_commands_complete = True # 초기화 수행 동작 완료 플래그(약간 중복인 것 같기도,,)
            ...
            if self.red_ball_detection:
                ...
                if self.initial_commands_complete and not self.position_reached:
                    if position == "Center" and self.position_started:
                        self.position_reached = True
                        print(f"{self.position_started.lower()}2center") #왼쪽에서 or 오른쪽에서 시작했는데 센터 맞췄다. 아니면 걍 센터엿거나
