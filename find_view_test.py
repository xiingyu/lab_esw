    ...
    def process_frame(self):
        ...
        if self.red_ball_detection:
            ...
            if self.initial_commands_complete and not self.position_reached:
                if position == "Left":
                    # Left 상태에서 1180 명령 전송
                    self.position_started = "Left"
                    self.serial_comm.send_data(1180)
                    self.delay(1)
                    print("Sent command: 1180 to adjust leftward")
                    ...
                    # 위치에 따라 추가 명령 전송
                    if position == "Left":
                        self.serial_comm.send_data(1140)
                        print("Sent command: 1140 to adjust further leftward")
                    elif position == "Center":
                        self.serial_comm.send_data(11100)
                        print("Sent command: 11100 to adjust to center")
