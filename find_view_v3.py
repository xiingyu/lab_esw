...
class MainAlgo:
    ...
    def process_frame(self):
        """ 공의 위치를 조정하기 위한 프로세스 """
        if self.red_ball_detection:  # 공이 있을 때
            position = self.check_position(self.red_circle, self.img_size_x)

            if self.initial_commands_complete and not self.position_reached:  # 첫 프레임에서 초기 목각도 설정 완료 및 목표 도달 아님 상태
                if position == "Left":
                    # Left 상태에서 초기 명령 전송
                    self.position_started = "Left"
                    current_command = 1190
                    
                    while position == "Left" and current_command >= 1120:  # 최솟값까지 반복
                        self.serial_comm.send_data(current_command)
                        #self.delay(1)
                        print(f"Sent command: {current_command} to adjust leftward")
                        ...
                        # 위치에 따라 추가 명령 전송
                        if position == "Left":
                            current_command -= 10  # 명령 값 감소 후 루프 반복
                        elif position == "Center":
                            print("Set :: left 2 center")
                            self.position_reached = True
                            break  # 목표에 도달하면 반복 종료
                        else:  # Right일 때
